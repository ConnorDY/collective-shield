...
class NewRequestView extends Component {
    ...
    render() {
        const options = [
            'Doctor', 'Nurse', 'First Responder', 'Medical Support Staff'
        ];

        const states = [
            'AL', 'AK', 'AZ', 'AR', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA', 'HI', 'ID', 'IL', 'IN', 'IA', 'KS', 'KY', 'LA', 'ME', 'MD', 'MA', 'MI', 'MN', 'MS', 'MO', 'MT', 'NE', 'NV', 'NH', 'NJ', 'NM', 'NY', 'NC', 'ND', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VT', 'VA', 'WA', 'WV', 'WI', 'WY', 'DC', 'MH'
        ];
                            ...
                            <form onSubmit={this._createNewRequest}>
                                ...
                                <fieldset>
                                    <textarea
                                        ...
                                        value={this.state.details}></textarea>
                                </fieldset>
    ...
    _createNewRequest(e) {
        ...
        axios.post(buildEndpointUrl('request', '/public'), data)
            .then(res => {
                this.setState({
                    isCreated: true
                })
    ...
    _validateRequest() {
        ...
        if (this.state.line1 == null || this.state.line1.length === 0
            || this.state.city == null || this.state.city.length === 0
            || this.state.state == null || this.state.state.length === 0
            || this.state.zip == null || this.state.zip.length === 0) {
            isValid = false
            this.setState({
